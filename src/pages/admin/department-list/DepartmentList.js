  ...
  const handleDelete = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_NODE_URL}department/delete?id=${event._id}`,
        {
          method: "GET",
          headers: {
          ...
          <table className="w-full table-auto border-collapse">
            <thead>
              <tr>
                ...
                <th className="border px-4 py-2">Program Offered</th>
              ...
              {newsAndEvents.length > 0 ? (
                newsAndEvents.map((event) => (
                  <tr key={event.id}>
                    ...
                    <td className="border px-4 py-2">{event?.school?.name}</td>
                    <td className="border px-4 py-2">
                    ...
                    <td className="border px-4 py-2">
                      <div className="flex justify-center space-x-2">
                        <button
                          onClick={() =>
                            router.push(
                              `/admin/edit-department?_id=${event?._id}`
                            )
                          }

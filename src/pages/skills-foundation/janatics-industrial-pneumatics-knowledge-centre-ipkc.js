...
// Data Structure
const janaticsData = {
    title: "JANATICS - Industrial Pneumatics Knowledge Centre (IPKC)",
    description: [
        "AKG Engineering College collaboration with Janatics strengthens industry–academia integration by providing hands-on training in pneumatic, electro-pneumatic, and PLC-based automation. The lab equips students with skills needed for modern manufacturing and smart factory environments. It is actively used by the Robotics/JARC club for project development and for preparing teams for competitions such as ROBOCON and JASC.",
        "The AKGEC-IPKC houses a wide range of pneumatic components for practical learning, including training benches, feeder station DIY kits, and an innovative pneumatic piano. These facilities provide hands-on exposure to industrial pneumatics, helping students understand air preparation, control valves, actuators, and automation concepts through real-time demonstrations and interactive learning setups."
    ],
    infrastructure: {
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <Header
                ...
                subHeading="Strengthening industry–academia integration with hands-on training in modern manufacturing technologies."
            ...
            <main className="pb-20">
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="prose prose-lg max-w-none text-gray-700 font-novaReg leading-relaxed">
                                {janaticsData.description.map((para, i) => (
                ...
                <section className="px-6 lg:px-8 max-w-[1450px] mx-auto mb-20">
                    <div className="mb-16 text-center">
                        ...
                        >
                            {janaticsData.infrastructure.title}
                        </motion.h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-8 justify-center">
                        {janaticsData.infrastructure.items.map((item, index) => {
                ...
                {/* Events Section */}
                {janaticsData.events.items.length > 0 ? (
                    <section className="px-6 lg:px-8 max-w-[1450px] mx-auto mb-20">
                        <div className="mb-12 text-center">
                            <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{janaticsData.events.title}</h2>
                            <div className="w-24 h-1 bg-brand-yellow mx-auto rounded-full"></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center">
                            {janaticsData.events.items.map((event, idx) => (
                                <div
                                    key={idx}
                ...
                {/* Awards Section */}
                {janaticsData.awards.items.length > 0 ? (
                    <section className="px-6 lg:px-8 max-w-7xl mx-auto mb-20">
                        <div className="mb-12 text-center">
                            <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{janaticsData.awards.title}</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 justify-center">
                            {janaticsData.awards.items.map((award, idx) => (
                ...
                <section className="bg-brand-blue text-white py-20 mb-20">
                    <div className="max-w-7xl mx-auto px-6 lg:px-8">
                        <div className="mb-12 text-center">
                            <h2 className="text-3xl md:text-4xl font-novaBold mb-6">{janaticsData.programs.title}</h2>
                            <p className="max-w-3xl mx-auto text-blue-100 font-novaReg text-lg">{janaticsData.programs.description}</p>
                        </div>
                ...
                {janaticsData.upcoming.table && janaticsData.upcoming.table.length > 0 ? (
                    <section className="max-w-7xl mx-auto px-6 lg:px-8 mb-20">
                        <div className="mb-12">
                            <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{janaticsData.upcoming.title}</h2>
                            {janaticsData.upcoming.description && (
                                <p className="text-gray-600 font-novaReg text-lg mb-8">{janaticsData.upcoming.description}</p>
                            )}
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {janaticsData.upcoming.table.map((row, idx) => (
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto text-left">
                                <div className="flex items-start gap-5">
                                    ...
                                    <p className="text-gray-300 text-lg leading-relaxed">{janaticsData.contact.address}</p>
                                </div>
                                <div className="flex flex-col gap-6">
                                    <div className="flex items-start gap-5">
                                        <Mail className="text-brand-yellow shrink-0 mt-1" size={24} />
                                        <a href={`mailto:${janaticsData.contact.email}`} className="text-gray-300 text-lg hover:text-white transition-colors">{janaticsData.contact.email}</a>
                                    </div>
                                    <div className="flex items-start gap-5">
                                        <Phone className="text-brand-yellow shrink-0 mt-1" size={24} />
                                        <div className="text-gray-300 text-lg leading-relaxed">
                                            {janaticsData.contact.phones.join(" / ")}
                                        </div>
                                    </div>
                                </div>
                            </div>

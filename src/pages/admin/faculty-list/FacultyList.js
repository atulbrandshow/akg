  ...
  const handleDelete = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_NODE_URL}faculty/delete?_id=${event._id}`,
        {
          method: "GET",
          headers: {
              ...
              {newsAndEvents.length > 0 ? (
                newsAndEvents.map((event) => (
                  <tr key={event.id}>
                    ...
                    <td className="border px-4 py-2">{event?.designation}</td>
                    <td className="border px-4 py-2">{event?.email}</td>
                    {/* <td className="border px-4 py-2">
                      {event.programsOffered?.map((program) => (
                        <span className="p-2 bg-green-300 rounded-lg text-gray-600">
                          {program}{" "}
                        </span>
                      ))}
                    </td> */}
                    <td className="border px-4 py-2">{event.phoneNumber}</td>
                    <td className="border px-4 py-2">{event.address}</td>
                    <td className="border px-4 py-2 whitespace-nowrap">
                      {event?.dateOfBirth?.split("T")[0]}
                    </td>
                    <td className="border px-4 py-2">
                    ...
                    <td className="border px-4 py-2">
                      <div className="flex justify-center space-x-2">
                        <button
                          onClick={() =>
                            router.push(`/admin/edit-faculty?_id=${event?._id}`)
                          }

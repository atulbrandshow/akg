...
export default function NewNavBar() {
  ...
  const LinksList = ({
    ...
    <div className="w-fit max-lg:mb-5 lg:mr-10 max-md:w-full">
      {title && (
        <h3
          className={`font-novaBold text-base text-blue-900 leading-none mb-3 ${titleClassName}`}
        >
          {title}
        </h3>
      )}
  ...
  return (
    <header
      className={`navbar z-[100] w-full fixed top-0 left-0 transition-all duration-200 ${
        isScrolled ? "bg-blue-700" : "bg-white/0"
      }`}
    >
      <div
        className={`hidden md:flex overflow-hidden w-full transition-all duration-300 ease-in-out ${
          isScrolled ? "-mt-4" : "py-1.5"
          ...
          <li className="relative group">
            ...
            <div
              className={`${
                ...
                openMenu === "skills-foundation" &&
                "absolute h-auto mt-0 w-full"
              } right-0 h-0 mt-5 overflow-hidden lg:group-hover:h-auto lg:group-hover:mt-0 transition-all bg-white
                                    text-black rounded-lg shadow-lg `}
            >
              <div className="grid grid-cols-2">
                <div className="col-span-2 transition-all">
                  <div className="flex p-5 group-hover:p-5 transition-all max-md:max-h-72 max-md:overflow-y-scroll max-md:flex-col">
                    {Object.keys(SkillsFoundation.sublinks).map(
                      (key, index) => (
                        <div key={index} className="w-52 pr-2">
                          <LinksList
                            title={key}
                            links={SkillsFoundation.sublinks[key]}
                            setBigMenuToggle={setBigMenuToggle}
                          />
                        </div>
                      )
                  ...
                  <div className="w-full relative max-md:hidden">
                    <div
                      ...
                      style={{
                        backgroundImage:
                          "url('/image/lab/User-Manual-AKGEC 5.webp')",
                      }}
                    >
                      <div className="absolute inset-0 bg-black opacity-70 rounded-lg"></div>
                    </div>

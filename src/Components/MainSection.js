...
const MainSection = ({ data }) => {
    ...
    useEffect(() => {
        if (data) {
            setExtraComponentData(data?.extraComponentData?.holder1)
        }
    }, [data])
                ...
                <div className="grid grid-cols-2 max-lg:grid-cols-1 max-lg:pb-10">
                    <div className="slide flex justify-center max-w-3xl flex-col pl-20 max-lg:mt-52 max-lg:items-center max-lg:px-5 ">
                        <h1 className="max-w-4xl font-novaThin max-sm:text-3xl tracking-tight text-white max-xl:text-5xl text-6xl max-lg:text-center max-[350px]:text-2xl">
                            {extraComponentData?.param?.split("|")[0]} <br />
                            <span className="relative whitespace-nowrap ">
                                ...
                                <span className="relative font-novaSemi text-white uppercase"> {extraComponentData?.param?.split("|")[1]}</span>
                            </span>
                        </h1>
                        <p
                            className="mt-8 text-white text-2xl font-novaReg max-lg:text-base max-lg:text-center"
                            dangerouslySetInnerHTML={{ __html: extraComponentData?.paramDesc }}
                        ></p>
                        <div className="h-20">
                            <Button onClick={() => { router.push(extraComponentData?.paramUrl||'/') }} text={'Apply Today'} className="py-3 max-sm:px-6 max-sm:text-xs px-10 mt-5 text-[15px] rounded-xl font-novaBold uppercase bg-btn-gradient animate-gradient text-white w-max  hover:bg-[#3c5686] hover:border-b-4 hover:border-[#beb6ff] hover:transform  scale-y-105 tracking-widest" />
                        </div>
                    </div>

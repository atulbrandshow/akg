...
export default function ExploreCourses({ data }) {
    ...
    const [extraComponentData, setExtraComponentData] = useState(data?.extraComponentData?.holder3);
    const [stats, setstats] = useState([])
    const [posts, setPosts] = useState([])
    useEffect(() => {
        if (data) {
            setExtraComponentData(data?.extraComponentData?.holder3);
            setstats(data?.extraComponentData?.holder3?.extraData[0]?.extraData || [])
            setPosts(data?.extraComponentData?.holder3?.extraData[1]?.extraData || [])
        }
    }, [data])
    return (
        <section className="bg-primary py-10 sm:py-16 md:py-20">
            <div className="break2:max-w-[1320px] break3:max-w-[1140px] break4:max-w-[960px] mx-auto px-4 sm:px-6 lg:px-8 text-white">
                <h2 className="text-[42px] max-lg:text-4xl max-md:text-3xl font-novaLight text-center tracking-tight">
                    {extraComponentData?.param.split("|")[0]} <span className='font-novaSemi bg-gradient-to-r from-red-300 to-teal-500 bg-clip-text text-transparent animate-gradient'>{extraComponentData?.param.split("|")[1]} </span>
                </h2>
                <p className="mt-4 sm:mt-5 text-base sm:text-lg md:text-xl lg:text-[22px] leading-snug text-gray-200 mx-auto max-w-xl sm:max-w-2xl md:max-w-3xl lg:max-w-screen-lg font-novaLight text-center"  dangerouslySetInnerHTML={{ __html: extraComponentData?.paramDesc }}/>
                        ...
                        {stats?.map((stat) => (
                            <div key={stat._id} className="mx-auto flex max-w-xs flex-col py-4">
                                ...
                                <dt
                                    ...
                                    dangerouslySetInnerHTML={{ __html: stat?.paramDesc || '' }}
                                ></dt>
                            </div>
                        ))}
                    </dl>
                ...
                <div className="mx-auto grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:gap-5 " >
                    {posts?.map((post) => (
                        <article key={post._id} className="relative bg-gray-900 rounded-lg shadow-md overflow-hidden">
                            <img
                                alt=""
                                src={post.paramImg[0]}
                                ...
                                <div className="flex flex-col ">
                                    <div className="h-16 max-xl:h-24 max-md:h-12">
                                        ...
                                        <p
                                            ...
                                            dangerouslySetInnerHTML={{ __html: post?.paramDesc || '' }}
                                        ></p>

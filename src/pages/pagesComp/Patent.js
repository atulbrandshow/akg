            ...
            <div className="bg-[#f4f4f4] p-8 md:p-12 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    {patentsData.map((patent, index) => (
                        <div key={index}>
                            <h3 className="text-[#1c2b4d] text-lg font-semibold font-novaReg mb-4">
                                {patent.title}
                            </h3>
                            <Link href={patent.link} target="_blank" rel="noopener noreferrer" className="text-[#c1272d] text-sm font-medium hover:underline inline-flex items-center">
                                Know More <span className="ml-1 text-lg leading-none">&rarr;</span>
                            </Link>
                        </div>
                    ))}
                </div>
                        ...
                        <div key={tab} className={activeTab === tab ? "block" : "hidden"}>
                            ...
                            <ul className="text-gray-700 space-y-3">
                                {publicationsData[tab].map((item, index) => (
                                    <li key={index} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-300 pb-3">
                                        <span className="flex items-center before:content-['•'] before:mr-2 before:text-[#c1272d] before:text-xl">
                                            {item.title}
                                        </span>
                                        <Link href={item.link} target="_blank" rel="noopener noreferrer" className="text-[#c1272d] text-sm font-medium hover:underline inline-flex items-center sm:ml-4">

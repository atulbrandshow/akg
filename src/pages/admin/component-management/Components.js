        ...
        <form className="space-y-5">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-4 py-2 border border-gray-300 font-normal rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
            />
            <p className="text-xs text-gray-500">
              Component Type will be:{" "}
              {editFormData?.componentName ? kebabToPascalCase(editFormData.componentName) : "PascalCase"}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={handleInputChange}
                required
                className="w-full px-4 py-2 border border-gray-300 font-normal rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white"
              >
...
export default function ComponentForm() {
  ...
  const handleSaveEdit = async (updatedData) => {
    try {
      const response = await fetch(`${API_NODE_URL}components/${editingComponent._id}`, {
        ...
        body: JSON.stringify({ ...updatedData, editedby: "user" }),
      })
  ...
  const generateSampleData = (type) => {
    const samples = {
      ...
      calculator: {
        componentName: "sample-calculator",
        category: "Page",
      },
    }
      ...
      <div className="">
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-purple-700 px-8 py-6">
            ...
            <p className="text-blue-100 font-normal text-base">
              Add, edit, and manage your application components with ease.
            </p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-7 gap-2 p-4">
            ...
            <div className="lg:col-span-2 space-y-6 bg-gray-50 p-4 rounded-xl shadow-inner">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-800">Add New Component</h2>
                <div className="flex whitespace-nowrap gap-1">
                  <button
                    type="button"
                    onClick={() => generateSampleData("holder")}
                    className="px-4 py-1.5 text-xs bg-blue-100 text-blue-800 rounded-full hover:bg-blue-200 transition-colors duration-200 font-semibold"
                  >
                    Holder Sample
                  </button>
                  <button
                    ...
                    className="px-4 py-1.5 text-xs bg-green-100 text-green-800 rounded-full hover:bg-green-200 transition-colors duration-200 font-semibold"
                  >
                    Page Sample
                  </button>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-purple-700 text-white py-3 px-4 rounded-lg font-bold hover:from-blue-700 hover:to-purple-800 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
                    <>
              ...
              {message.text && (
                <div
                  className={`p-4 rounded-lg text-sm font-normal ${
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <h2 className="text-2xl font-bold text-gray-800">Components ({total})</h2>
                {/* Search */}
                <form onSubmit={handleSearch} className="flex gap-3 w-full sm:w-auto">
                  <div className="relative flex-grow">
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2 border font-normal border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                    />
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                  </div>
                  <button
                    type="submit"
                    className="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 font-normal"
                  >
                    Search
                  </button>
                </form>
                    ...
                    {components.map((component) => (
                      ...
                      >
                        <div className="flex-1 flex gap-2 items-end">
                          <div>
                            <h3 className="font-bold text-gray-900 text-lg">{component.componentType}</h3>
                ...
                <div className="flex items-center justify-center gap-3 mt-6">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    <button
                      ...
                      className={`px-4 py-2 border rounded-lg transition-colors duration-200 font-normal ${
                        currentPage === page

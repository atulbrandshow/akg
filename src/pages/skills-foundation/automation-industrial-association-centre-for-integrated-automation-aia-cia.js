...
// Data Structure
const aiaData = {
    title: "Automation Industrial Association - Centre for Integrated Automation (AIA-CIA)",
    description: [
        "AKG Engineering College (AKGEC), in partnership with the Automation Industry Association (AIA), has set up India’s first Competency Development Center for Integrated Automation under the AIA Campus Connect Initiative. Established on 26 June 2014, the center operates with support from leading automation companies such as B&R, FESTO, SIEMENS, and Pepperl & Fuchs, offering hands-on training in advanced industrial automation technologies."
    ],
    infrastructure: {
        title: "Infrastructure at AIA-CIA",
        items: [
            {
                id: "1",
    ...
    upcoming: {
        ...
        description: "Team Automation is offering an 80 Hrs Winter Training Program in Industrial Automation—Basic and Advanced Levels for B.Tech and Diploma students, scheduled for December 2025 and January 2026.",
        contactEmail: "cia@akgec.ac.in",
            ...
            <main className="pb-20">
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="prose prose-lg max-w-none text-gray-700 font-novaReg leading-relaxed">
                                {aiaData.description.map((para, i) => (
                ...
                {/* <section className="px-6 lg:px-8 max-w-[1450px] mx-auto mb-20">
                    <div className="mb-12 text-center">
                        <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{aiaData.events.title}</h2>
                        <div className="w-24 h-1 bg-brand-yellow mx-auto rounded-full"></div>
                    </div>
                    {aiaData.events.items.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {aiaData.events.items.map((event, idx) => (
                                <div
                                    key={idx}
                        ...
                        <div className="text-center text-gray-500 italic">No events currently listed.</div>
                    )}
                </section> */}
                ...
                {/* <section className="px-6 lg:px-8 max-w-7xl mx-auto mb-20">
                    <div className="mb-12 text-center">
                        <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{aiaData.awards.title}</h2>
                        <div className="w-24 h-1 bg-brand-blue mx-auto rounded-full"></div>
                    </div>
                    {aiaData.awards.items.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {aiaData.awards.items.map((award, idx) => (
                        ...
                        <div className="text-center text-gray-500 italic">No awards currently listed.</div>
                    )}
                </section> */}
                ...
                <section className="bg-brand-blue text-white py-20 mb-20">
                    <div className="max-w-7xl mx-auto px-6 lg:px-8">
                        <div className="mb-12 text-center">
                            <h2 className="text-3xl md:text-4xl font-novaBold mb-6">{aiaData.programs.title}</h2>
                            <p className="max-w-3xl mx-auto text-blue-100 font-novaReg text-lg">{aiaData.programs.description}</p>
                        </div>
                ...
                <section className="max-w-7xl mx-auto px-6 lg:px-8 mb-20">
                    <div className="mb-12">
                        <h2 className="text-3xl md:text-4xl font-novaBold text-gray-900 mb-4">{aiaData.upcoming.title}</h2>
                        <p className="text-gray-600 font-novaReg text-lg mb-8">{aiaData.upcoming.description}</p>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {aiaData.upcoming.table.map((row, idx) => (
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto text-left">
                                <div className="flex items-start gap-5">
                                    ...
                                    <p className="text-gray-300 text-lg leading-relaxed">{aiaData.contact.address}</p>
                                </div>
                                <div className="flex flex-col gap-6">
                                    <div className="flex items-start gap-5">
                                        <Mail className="text-brand-yellow shrink-0 mt-1" size={24} />
                                        <a href={`mailto:${aiaData.contact.email}`} className="text-gray-300 text-lg hover:text-white transition-colors">{aiaData.contact.email}</a>
                                    </div>
                                    <div className="flex items-start gap-5">
                                        <Phone className="text-brand-yellow shrink-0 mt-1" size={24} />
                                        <div className="text-gray-300 text-lg leading-relaxed">
                                            {aiaData.contact.phones.join(" / ")}
                                        </div>
                                    </div>
                                </div>
                            </div>

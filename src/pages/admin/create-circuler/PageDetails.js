...
const router = useRouter();
  ...
  const handleShortDescChange = (editorState) => {
    setShortDescEditorState(editorState);
    const content = convertToRaw(editorState.getCurrentContent());
    setFormData(prev => ({
      ...prev,
      shortdesc: JSON.stringify(content)
    }));
  };

  const handleDescChange = (editorState) => {
      ...
      <section className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="border rounded">
              <Editor
                editorState={shortDescEditorState}
                onEditorStateChange={handleShortDescChange}
                toolbar={{

...
const TableList = ({ type, title, subTitle }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `${API_NODE_URL}slug/getbytype?type=${type}`,
        {
          credentials: "include",
        },
      );
      ...
      const homePageItems = fetchedData.filter(
        (item) => item.ComponentType === "HomePage",
      );

      // Then, separate the rest (non-HomePage)
      const otherItems = fetchedData.filter(
        (item) => item.ComponentType !== "HomePage",
      );
  ...
  const uniqueTypes = [
    "All",
    ...new Set(newsAndEvents.map((item) => item.type)),
  ];
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-novaBold text-gray-900">
                  {title}
                </h1>
            ...
            <div className="text-right">
              ...
              <p className="text-2xl font-novaBold text-gray-900">
                      ...
                      >
                        <td className="px-2 py-4">
                          <div className="flex items-center">
                            <div className="ml-4">
                              <div className="text-sm font-novaSemi text-gray-900 line-clamp-2">
                                {event.name}
                              </div>
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col space-y-1">
                            <Link
                              target="_blank"
                              href={event.path}
                              className="text-xs text-nowrap flex gap-1 items-center w-fit bg-blue-100 px-2 py-1 rounded text-gray-800 font-novaSemi hover:bg-gray-200 "
                            >
                              <View size={14} />
                              View Page
                            </Link>
                            <span className="text-xs font-novaSemi text-gray-500">
                              ID: {event.page_id}
                            </span>
                          </div>
                        </td>
                              ...
                              hasPermission(type, "create") && (
                                <button
                                  onClick={() => {
                                    ...
                                    router.push(
                                      `/admin/add-faq?page_id=${event?.page_id}`,
                                    );
                                  }}
                              ...
                              hasPermission(type, "create") && (
                                <button
                                  onClick={() => {
                                    ...
                                    router.push(
                                      `/admin/add-testimonial?page_id=${event?.page_id}`,
                                    );
                                  }}
                                  className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-novaSemi rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 transition-all duration-200 transform hover:scale-105"
                                  title="Testimonial"
                              ...
                              hasPermission(type, "create") && (
                                <button
                                  onClick={() => {
                                    ...
                                    router.push(
                                      `/admin/add-review?page_id=${event?.page_id}`,
                                    );
                                  }}
